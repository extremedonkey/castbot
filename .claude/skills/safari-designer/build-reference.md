...
- **Must ALSO be in the map's `blacklistedCoordinates`** or the item does nothing (legend warns). Binary (qty 1 = unlock), global to all holders, not consumed. **AND across items**: a cell listed on multiple items' reverse blacklists needs ALL of those items held to enter — this is the native multi-key door; holding a subset leaves it locked with no player-facing cue. Validation server-side in `movePlayer`; coverage `computeReverseBlacklistCoverage` / `getPlayerReverseBlacklistCoverage` (`mapMovement.js`). Reverse-blacklist **wins** over blacklist. Movement still requires adjacency.
