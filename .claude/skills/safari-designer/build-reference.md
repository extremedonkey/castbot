...
- Data: `safariContent[guildId].maps.active` (id pointer) + `maps[mapId]` = {gridWidth, gridHeight, imageFile, category, blacklistedCoordinates[], coordinates{ COORD: {channelId, anchorMessageId, emoji, baseContent{title,description,image}, stores[], buttons[] }}}  (legacy itemDrops[]/currencyDrops[] may still exist in old data — the feature was REMOVED 2026-08-02, nothing reads them). Player progress in `playerData[guildId].players[uid].safari.mapProgress[mapId]` = {currentLocation, exploredCoordinates[], itemsFound[], movementHistory[]}.
...
- Cell authoring: in the cell's channel anchor → 📍 Location Actions `map_location_actions_{coord}` (`app.js:33409`) opens the map_cell entity editor. Field groups (`entityManagementUI.js:700`): `info` (title/description/image/emoji), `stores`. Quick-create: `quick_text_{coord}`, `quick_currency_`, `quick_item_`, `quick_itemtext_`, `quick_enemy_`, `quick_command_` (label "CmdText": command → display_text), `quick_cmditem_` (label "CmdItem": command → give_item), `quick_crafting_`. ⚠️ Map Drops was REMOVED 2026-08-02 — use Quick Items / Quick Currency instead (`quick_item_`, `quick_currency_`).
...
- **Must ALSO be in the map's `blacklistedCoordinates`** or the item does nothing (legend warns). Binary (qty 1 = unlock), global to all holders, not consumed. **Multi-item cells are mode-dependent** — `safariConfig.reverseBlacklistRequireAll` (Settings → 📍 Location → "Require ALL Key Items"): unset/false = **OR** (any one listing item unlocks — the legacy default); true = **AND** (must hold every listing item = native multi-key door, subset stays locked with no player-facing cue). Single-item cells behave the same either way. Validation server-side in `movePlayer`; coverage `computeReverseBlacklistCoverage(inventory, items, requireAll)` / `getPlayerReverseBlacklistCoverage` (`mapMovement.js`). Reverse-blacklist **wins** over blacklist. Movement still requires adjacency.
...
- Admin overview: 🚀 Safari Progress `safari_progress` (`safariProgress.js`) — per-row view of actions and claims (read-only audit).
