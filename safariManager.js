...
// Button style to numeric value mapping (for Components V2 raw format)
function getButtonStyle(style) {
    const styleMap = {
        ...
        'Success': 3,
        'Danger': 4,
        1: 1, 2: 2, 3: 3, 4: 4  // Already numeric
    };
...
async function createPlayerInventoryDisplay(guildId, userId, member = null, currentPage = 0) {
    try {
        ...
        if (inventoryActions.length > 0) {
            ...
            const tempContainer = { type: 17, components: components };
            const currentComponentCount = countComponents([tempContainer], { enableLogging: false });

            const DISCORD_LIMIT = 40;
            const SAFETY_BUFFER = 4;  // Reserve space for pagination nav row
            const MAX_BUDGET = DISCORD_LIMIT - SAFETY_BUFFER;  // 36
            const remainingBudget = MAX_BUDGET - currentComponentCount;

            console.log(`📊 Inventory Actions: ${inventoryActions.length} actions, budget: ${currentComponentCount} used, ${remainingBudget} remaining`);

            // Need at least: separator(1) + header(1) + actionRow(1) + 1 button = 4 components minimum
            if (remainingBudget >= 4) {
                // Calculate how many actions we can fit
                // ActionRow with 5 buttons = 6 components (1 row + 5 buttons)
                const headerCost = 2;  // separator + header text
                const availableForButtons = remainingBudget - headerCost;
                const maxRows = Math.floor(availableForButtons / 6);  // Each row = 1 row + 5 buttons max
                const maxActions = Math.min(maxRows * 5, inventoryActions.length);

                if (maxActions > 0) {
                    // Add separator before actions section
                    components.push({ type: 14 });

                    // Add header with truncation info if needed
                    components.push({
                        type: 10,
                    ...
                    // Sort actions by sortOrder if defined, otherwise by name
                    const sortedActions = inventoryActions
                        .slice(0, maxActions)
                        .sort((a, b) => {
                        ...
                        const buttonRow = {
                            ...
                            components: rowActions.map(action => {
                                const button = {
                                    ...
                                    style: getButtonStyle(action.inventoryConfig?.buttonStyle || action.trigger?.button?.style || 'Secondary')
                                };
                    ...
                    console.log(`✅ Added ${Math.min(maxActions, sortedActions.length)} inventory action buttons`);
