...
- **📅 SEASON PLANNER** → [docs/03-features/SeasonPlanner.md](docs/03-features/SeasonPlanner.md) - The 📅 **Planner** tab inside Season Manager. Four estimates (players/swaps/FTC/start date, saved **incrementally**) generate `seasonRounds[seasonId]` — one round per F-number, one challenge each. Owns the **day arithmetic**: `getRoundDuration` → `calculateRoundDates` → the per-round dates rendered in the round string selects (`tribalDays` is an offset **from the challenge day**; `0` = live/same-day). ⚠️ The day logic is **duplicated** in [scheduleImageGenerator.js](scheduleImageGenerator.js) and the copies have diverged — change both. Core: [seasonPlanner.js](seasonPlanner.js). Was RaP 0947
