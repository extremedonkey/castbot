...
- **📅 SEASON PLANNER** → [docs/03-features/SeasonPlanner.md](docs/03-features/SeasonPlanner.md) - The 📅 **Planner** tab inside Season Manager. Four estimates (players/swaps/FTC/start date, saved **incrementally**) generate `seasonRounds[seasonId]` — one round per F-number, one challenge each. **All day arithmetic lives in [seasonRoundSchedule.js](seasonRoundSchedule.js)** — one pure module (`getRoundType` → `getRoundPhases` → `getRoundDuration` → `buildRoundSchedule`), imported by BOTH the round string selects ([seasonPlanner.js](seasonPlanner.js)) and the Schedule/Calendar images ([scheduleImageGenerator.js](scheduleImageGenerator.js)). `tribalDays` is an offset **from the challenge block's last day** (`0` = live/same-day); `challengeDays` is that block's length — a **shared budget** for the main challenge plus an optional linked **bonus/reward challenge** (`bonusChallengeId` + `bonusOrder`), so adding a reward never lengthens a round. 🔴 **Never reimplement day logic in a consumer** — a duplicated copy is what silently desynced the images from the selects. Was RaP 0947
- **📝 SEASON APPLICATIONS** → [docs/03-features/SeasonAppBuilder.md](docs/03-features/SeasonAppBuilder.md)
- **📜 GOOGLE SHEETS SYNC** → [docs/03-features/GoogleSheetsSync.md](docs/03-features/GoogleSheetsSync.md) - Push a Google Form response sheet into a season for hosts who recruit **off-Discord** (IRL leagues, Instagram funnels). Creates one application channel per row so Casting/Marooning work for applicants with no Discord account. Prescribed headers (`Name`/`Age`/`Pronouns` mapped; everything else dumped as Q&A pairs, stored nowhere), HMAC-signed push endpoint (`/api/sheets-sync`), generated Apps Script, dry-run preview confirmed in Sheets, per-column privacy exclusions. Create-only and one-way — **post-import edits never propagate**. Code: [src/sheets/](src/sheets/)
...
- **📂 CHANNEL ARCHIVE + ☢️ NUKE CHANNELS** → [docs/03-features/ChannelArchive.md](docs/03-features/ChannelArchive.md) - Archive a channel/category's full history as on-Discord HTML (rate-limit-safe REST fetch, type-13 file, htmlpreview link). Four Archive Modes across `embed` × `deletes` (**Full Archive** is the default; the two `+ Delete Channels` modes reclaim channel slots). **All destructive deletion goes through [channelNuker.js](channelNuker.js)** — one engine for Archive & Delete *and* ☢️ Nuke Channels (`prod_nuke_category`, channels AND categories). 🔴 Never delete a channel outside `deleteChannelItems`/`verifyThenDelete`: the invoking channel is always protected, a channel dies only after its archive is re-fetched and confirmed, and a category survives anything left inside it. Code: [channelArchiver.js](channelArchiver.js), [channelExportFetcher.js](channelExportFetcher.js), [channelExport.js](channelExport.js)
