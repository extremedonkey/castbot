...
const features = [
  { icon: '🔐', title: 'Roles & Security', desc: 'Global permission system\ncontrol CastBot feature access by role', color: PURPLE, tag: 'NEW' },
  { icon: '🎒', title: 'Item-Triggered Actions', desc: 'Use inventory items to trigger\ncustom actions with bundled results', color: GREEN, tag: 'NEW' },
  { icon: '📜', title: 'Safari Log Backfill', desc: 'Parse and import historical\nSafari log channel data', color: BLUE, tag: 'NEW' },
  { icon: '🗺️', title: 'Activity Log Upgrades', desc: 'Better overlay visuals, decoupled\nfrom Safari channel settings', color: ORANGE, tag: 'IMPROVED' },
  { icon: '👤', title: 'Name Resolution', desc: 'Guild member fetch fallback\nfor reliable display names', color: CYAN, tag: 'FIXED' },
  { icon: '🔧', title: 'Parser & Stability', desc: 'Backfill parser fixes, mention\nformat handling, movement logs', color: PINK, tag: 'FIXED' },
];

const stats = { commits: 17, insertions: 4425, deletions: 201, files: 24 };
...
function buildSvg() {
  ...
  features.forEach((f, i) => {
    ...
    const tagColor = f.tag === 'NEW' ? GREEN : f.tag === 'IMPROVED' ? BLUE : ORANGE;
    ...
    cards += `
      ...
      <!-- Tag -->
      <rect x="${x + cardW - 82}" y="${y + 10}" width="70" height="20" rx="10" fill="${tagColor}" opacity="0.2" />
      <text x="${x + cardW - 47}" y="${y + 24}" font-family="Arial,sans-serif" font-size="10" font-weight="bold" fill="${tagColor}" text-anchor="middle">${f.tag}</text>
  ...
  return `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Subtitle -->
    <text x="${W/2}" y="140" font-family="Arial,sans-serif" font-size="16" fill="${GRAY}" text-anchor="middle">Roles &amp; Security, Item-Triggered Actions, Safari Log Backfill &amp; More</text>
...
async function generate() {
  ...
  const outPath = '/home/reece/castbot/temp/castbot-update-mar2026-v2.png';
