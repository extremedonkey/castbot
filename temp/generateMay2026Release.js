...
const cards = [
  { title: 'Player Claims',      badge: 'NEW',      desc: 'Per-player manager: see exactly who\nhas claimed each claimable outcome',        color: '#3498db' },
  { title: 'Manual Claim',       badge: 'NEW',      desc: 'Grant a claim to any player by hand,\nstraight from the manager',                color: '#1abc9c' },
  { title: 'Set Cooldown',       badge: 'NEW',      desc: 'Override a player\'s remaining cooldown\non timed outcomes — any duration',       color: '#9b59b6' },
  { title: 'Clear &amp; Reset',  badge: 'IMPROVED', desc: 'Clear one player or reset everyone —\nnow on the same screen as the view',       color: '#e67e22' },
  { title: 'Live Refresh',       badge: 'NEW',      desc: 'Refresh cooldown timers in place\nwithout reopening the menu',                  color: '#e91e63' },
  { title: 'Cooldown Overrides', badge: 'FIXED',    desc: 'Set cooldowns longer than the base\nperiod — and they display correctly',       color: '#e74c3c' },
];

const stats = [
  { value: '6',    label: 'COMMITS',       color: '#3498db' },
  { value: '+923', label: 'INSERTIONS',    color: '#2ecc71' },
  { value: '-149', label: 'DELETIONS',     color: '#e74c3c' },
  { value: '9',    label: 'FILES CHANGED', color: '#e91e63' },
];
...
function buildSvg() {
  ...
  const dateBadge = `
    ...
    <text x="${W/2}" y="59" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="12" font-weight="bold" fill="#9b59b6" letter-spacing="2">MAY 2026</text>
  `;
  ...
  const subtitle = `<text x="${W/2}" y="145" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="16" fill="${TEXT_SEC}">Player Claims — View, Manage &amp; Reset</text>`;
...
async function generate() {
  ...
  const outPath = '/home/reece/castbot/temp/castbot-update-may2026.png';
