...
## Addendum (2026-07-27 evening): The Killing Blow — What Actually Ate the Memory

Reece's PM2 error-channel screenshots (the `Stale safariContent read discarded (gen 168→181)` storm + repeated overlay upload failures, 09:44–09:57Z) prompted a log-level deep dive. The freeze was **not gradual drift alone — it was a self-reinforcing allocation spiral in the Thespi Hunt guild (1524773737973682267), and it is a code bug class, not pure capacity**:

1. **safariContent write amplification**: from 09:32Z two players ground the safari (~10 interactions/min). Every search/claim/stamina click **rewrites the entire 2.5MB `safariContent.json`** (claimedBy arrays and stamina live inline in it) — 17 full saves in ~13 min. Each save bumps the generation counter, discarding every in-flight read → **23 safariContent + 50 playerData full parses were thrown away and redone** (~272MB of raw wasted parse input; 3–5× that in object-graph churn). The incident-07 cache guard worked *correctly* — at the cost of re-parses the box couldn't afford.
2. **The activity-overlay pipeline** (per view-logs click): downloads the full 2988×2988 map PNG (**7.2MB** — just under the 7MiB compress threshold, so it re-uploads full-size), runs **2–3 full Sharp decode/encode cycles at ~36MB raw RGBA each** (including a redundant `sharp(buffer).toFile()` where an `fs.writeFile` of the already-encoded buffer would do), then pins two ~7.2MB buffers in closure scope through a 10–15s upload timeout. **No memoryGuard pre-flight** — incident 06's guard covers map builds + analytics; this path was missed.
3. **The lag→re-click multiplier**: 12 overlay renders fired in 13 min (4× player + 8× admin view-logs, same target user). Only 5 uploads succeeded; **7 failed (ECONNRESET/AbortError) showing the user *nothing*** — so they clicked again. Two orphan PNGs share the same second: concurrent renders, no single-flight guard. Interaction volume *accelerated* as the box died: 9 clicks in 09:00–09:20 → **38 in 09:41–09:50**.
