...
- **🚨 GOTCHA — Radio Group options take NO `emoji` field** (2026-08-02). No working Radio Group in this codebase carries one; an unsupported field rejects the entire modal with a bare "This interaction failed" and nothing server-side.
- **🚨 GOTCHA — emoji INSIDE option `label` strings can break `default` pre-selection** (observed 2026-08-05, Currency Visibility radio): a payload with a correct single `default: true` rendered with NOTHING selected when option labels led with emoji (`'🟢 Enabled (default)'`). Keep radio option labels and descriptions PLAIN TEXT — the stamina Regeneration Style radio (plain text) is the reference working shape. Enforced by tests/safariSettingsUI.test.js ("no emoji" invariant).
