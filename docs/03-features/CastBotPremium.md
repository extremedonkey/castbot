...
Why lock-swap instead of gating the real handlers: Premium-menu buttons shared custom_ids with the free Tools menu — handler-level gates would have paywalled Tools too. The swap is a **commercial** gate, not a security boundary; it's applied server-side at render, and the deliberately-open money path (`Donate`, `Get Premium`) never locks. Note `hasPremiumAccessSync` is **tier-only**: à-la-carte feature grants unlock their features (via `premium:` declarations) but not the premium menu surface.

**🪛 Tools was killed 2026-08-08** (Reece: "kill Tools right now") — the `castbot_tools` button is gone from `/menu`, making the Premium paywall the *only* path to what Tools used to give away (Attributes, Archiver, utilities, Ask CastBot's free-guild surface). `setup_menu` + handler remain temporarily for in-flight ephemerals. Guilds in **grace** see a computed nag line at the top of the Premium menu ("expired <t:R> — renew to keep it").

### 🎟️ Self-service Redeem (LIVE — email-claim, Lite model)
...
## 🎛️ Admin operations (CastBot Premium menu → 🎟️ Entitlements, grey Reece-only accessory on the header Section)
