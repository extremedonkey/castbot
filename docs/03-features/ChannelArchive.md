...
Errors are caught per-channel: a red error message is posted to the channel and the loop continues. After the run, if more than one channel was requested, a **final summary** is posted (`✅ N archived` / `⚠️ N archived, M failed`) so a run never ends silently. The summary is an **ephemeral interaction followup** (only the invoking user sees it). On runs that outlast the ~15-min interaction token, the followup returns **401 / `Invalid Webhook Token` (50027)** — this is *expected* (a 263-channel run took ~24 min), so it's logged as an `ℹ️` info line (not an error → the PM2 error logger ignores it) and the summary **falls back to a public post**.
