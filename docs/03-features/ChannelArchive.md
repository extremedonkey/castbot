...
- **`fetchImageData(allMessages, { maxWidth=1280, quality=72, maxBytes=2MiB })`** — Self-Contained mode: fetches image attachments and **recompresses each to WebP via `sharp`** (resize + quality), returning `{ url: data:image/webp;base64 }`. Recompression is what makes Self-Contained viable — a 1–3 MB screenshot → ~100–250 KB — so the file stays small *and* the bytes are baked in (survives source-channel deletion). Anything sharp can't decode, or still over `maxBytes` after compression, falls back to its (expiring) link.
...
- **`renderContent(text, ctx)` / `renderInline(text, ctx)`** — a small Discord-flavoured markdown renderer (replaced the old `markdownToHtml`). Handles bold/italic/underline/strike/inline-code/URLs, **fenced code blocks**, **headings (`#`/`##`/`###`)**, **`-#` subtext** (small muted line), **blockquotes/lists**, **spoilers** (`||…||`, click/hover to reveal), **custom emoji** (`<:name:id>` → CDN `<img>`), **timestamps** (`<t:unix:style>`), and **mentions** (see below). HTML-token outputs are stashed behind `\x00`/`\x01` sentinels before escaping and restored after — chosen specifically so bare digits in real text (e.g. "Top 5 players", code-block contents) can't collide with placeholders.
- **Image lightbox** — clicking any attachment/embed image opens a Discord-style full-screen preview pane (dark backdrop, centred image, *Open original ↗* link); closes on backdrop click, the **×** button, or **Esc**. Pure inline CSS/JS, no deps; works on both expiring-link and Self-Contained (base64) images. Emojis/avatars are excluded.
...
2. **All content exported as `[no content]`.** Message Content Intent was off in the Developer Portal → REST returned empty `content`. Fixed by enabling the **portal toggle** for CastBot-Dev (no code/gateway change). ⚠️ **PROD is different (discovered 2026-08-08, first prod archive):** production CastBot is now past the 100-guild verification threshold (142 guilds at time of writing), so its Message Content Intent is **not a free toggle** — it requires a privileged-intent request/approval from Discord (Portal → Bot → Privileged Gateway Intents). Until granted, prod archives render every message as `[no content]` (authors/reactions survive; `content`/`embeds`/`attachments` are redacted server-side). Workaround: run the archive with an app that has the intent (CastBot Test / CastBot-Dev invited to the target server).
...
- **CDN link expiry — solved.** Signed Discord URLs expire ~24h (no permanent schema exists for arbitrary files). Two-part fix: the **🔐 Unlock ⇄ 🔓 View** toggle (see [Link freshness](#-link-freshness-unlock--view)) means the online link is only ever shown freshly-minted and auto-reverts, so it's never stale; and the **🖼️ Self-Contained** Archive Mode embeds *inline images* as **WebP-recompressed base64** (via `sharp`) so they survive even **after the source channel is deleted** — and stay small. (URL-refresh can't help once the source is deleted: Discord deletes the source attachments, so the bytes must be copied in at archive time — hence the recompress-and-embed approach.) Avatars/emoji/default-avatars use unsigned CDN paths and never time-expire. The downloadable HTML attachment is permanent regardless. Non-image attachments still use expiring links (rare; not embedded).
