...
**Related:** [SeasonPlanner](SeasonPlanner.md) (the 📅 Planner tab, in full) · [SeasonAppBuilder](SeasonAppBuilder.md) · [SeasonLifecycle](../concepts/SurvivorContext.md) · [DNCOverview](DNCOverview.md) · RaP [0910 Unification](../01-RaP/0910_20260615_SeasonHubUnification_Analysis.md) · [0909 Search](../01-RaP/0909_20260615_SeasonSelectorSearch_Analysis.md) · [0908 Deletion](../01-RaP/0908_20260615_SeasonDeletion_Analysis.md)
...
| 📅 **Planner** | `apps_planner_${configId}` | Round/challenge timeline + estimates (see [SeasonPlanner.md](SeasonPlanner.md)) | `buildPlannerView` (seasonPlanner.js) |
...
A first-class Season Manager tab, rendered by `buildMarooningView`. Groups render in this order: **✅ Cast Players / 🔄 Alternate / ⚪ Undecided** (the "still in consideration" candidates — share ONE continuous numbering counter) → **🙅 Don't Cast / ✖️ Withdrawn** (each own numbering, restarting at 1; **hidden by default**, revealed via the **🗑️ Rejects** toggle button in the bottom row — `marooning_show_rejects_*` / `marooning_hide_rejects_*`, static label regardless of state). **Offer progress is per-row flags, not sub-sections** (2026-08-09). Cast Players and Alternate used to fan out into **- Accepted / - Offer Sent / - Draft** sub-blocks, which scattered each tribe's roster across three places — the opposite of what you need when balancing tribes. Now each section is ONE tribe-grouped list, and only *outstanding* work is marked: `⚠️✒️` no offer sent, `⚠️📨` sent but no reply, `⚠️🚫` declined. **Accepted carries no flag** — flagging every row would bury the exceptions. Within a tribe, rows sort by stage (accepted → awaiting → no offer), so anything needing action sinks to the bottom. A 🔑 **Key** block lists only the flags actually present. Undecided / Don't Cast / Withdrawn are unflagged (no offer is expected there). Every row is **one line**: `2. ReeceBot - 27yo | @Ask | @CST / CDT` — name plus inline demographics (`resolvePlayerDemographics`, shared with the Casting card's Overview block), with the `· 🎉 Accepted` / `· ✅ Accepted (Alt)` / `· 🚫 Declined` placement tags appended. **Scores and vote counts are NOT shown here** (removed 2026-08-09) — by marooning the decision is already made, so `5.0/5.0 (1 vote)` was noise, and pushing demographics onto a second `-#` line doubled the height of every roster. Ordering still runs highest-score-first via `computeCastingOrder`, so the ranking is preserved; the numbers live on the 🏆 Casting tab. Cast Players' header shows `(N/Est)` when the Season Planner's Estimated Number of Players is set (`estimatedTotalPlayers`, uncapped — exceeding it is valid). Grouping/sort is the shared `computeCastingOrder` (order: cast → alternative → undecided → reject → withdrawn; normalizes any unknown status → Undecided).

**Layout (reworked 2026-08-09).** Casting decisions lead the tab, under the header `### ```🚣‍♀️ Marooning Planner``` ` (was `🎬 Casting Decisions`) plus an intro line, then an action row of **👥 Add Cast** + **✒️ Bulk Offers** (renamed from Bulk Invites — the `casting_messages_0_*` custom_id is deliberately unchanged), then the roster. The 🏕️ Tribes section moved **below** the roster with its own explanatory copy. **🗑️ Rejects** stays in the shared bottom row next to Edit. The old **📊 SUMMARY** block (Total / Cast / Alternate / Undecided / Rejected / Withdrawn / Scored) was **deleted** — every figure in it already appears in the section headers directly above. The header/intro and the roster are now two separate Text Displays (the action row sits between them), which also relieves the latent 4000-char cap the single monolithic `body` string carried.

**👥 Add Cast** (`marooning_add_cast_*` → `marooning_add_cast_modal|{configId}`, [src/seasons/addCast.js](../../src/seasons/addCast.js)): creates application channels for cast who never clicked Apply — late additions, off-Discord recruits. A User Select (1–25) plus a mode select: *create + add the user to the channel* (default) or *create without adding them* (dry runs that don't ping real people, via `createApplicationChannel`'s `grantApplicantAccess` option). Delegates to `createApplicationChannel` rather than forking it, so the result is byte-identical to a real application: same permissions, welcome card, `currentQuestion: 1`, **no `castingStatus`**. Create-only — a user with a live application for this season is skipped and reported, never duplicated or overwritten. Paced through `runPacedJob`/`PACE_CREATE` with streamed progress, since channel create is one of Discord's tightest buckets.

**Tribes** (`getMarooningTribeRoleIds`, castRankingManager.js): simplified from the old 3-format castlist-membership check (`castlistIds[]` / `castlistId` / legacy `castlist` string, matched against `'default'`) to a direct existence check — **every tribe in `playerData[guildId].tribes` whose Discord role still exists**, full stop, regardless of which castlist(s) it's linked to. Fixes a real gap: a tribe created via the Reece-only "Tribes (Legacy)" debug flow has no `castlistIds` at all and could fail every one of the three format checks. 💭 Draft Tribes needs **≥1** known tribe (was ≥2 — dropped since drafting into a single tribe is still meaningful for planning).

**Tribe headers show size** — `<@&role> (4 players)`, replacing `(tentative)` (the Tribes blurb already states drafts are private, whereas tribe size is what you balance against). Two reconciliation blocks sit below the rosters: **⚠️ Drafted — No Application** (drafted via the modal's server-wide User Select but never applied, so they can't be scored or offered — 👥 Add Cast fixes it) and **⚠️ Duplicate Applications** (same person, multiple application channels in one season, inflating the counts — `createApplicationChannel`'s guard matches on channel *name*, so a rename lets someone re-apply; withdraw the extra channel to clear it).

**🔒 New Tribe is PRIVATE from Marooning (2026-08-09).** 🏕️ New Tribe reuses the Castlist Hub's `tribe_add_button|default|marooning_{configId}`, and that `marooning_` origin now does double duty: it routes the post-submit refresh back to Marooning **and** forces private creation — the modal drops its **Tribe Members** User Select, and the submit skips both `member.roles.add()` and `linkTribeToCastlist`, storing the tribe with `castlistIds: []` and no `castlist` key. Before this, creating a tribe from the Marooning tab assigned real Discord roles and published the tribe to the Active Castlist, broadcasting in-flux casting decisions to every spectator and cast member — the exact thing Draft Tribes was built to avoid, from the button sitting right next to it. The submit enforces the rule server-side, not just by hiding the field. The Castlist Hub flow is untouched and still does both. **Nothing converts `draftTribes` into real roles** — publishing is manual, by design.
...
- **✒️ Bulk Offers** (Marooning, `casting_messages_0_{configId}`) → modal with the 12-option "what to do" select, defined once in `BULK_MODE_OPTIONS` (castRankingManager.js). Two blocks:
  - **Send** — `all` / `successful` / `alternative` / `unsuccessful`. Values unchanged (they double as message-type keys inside `selectInviteTargets`); only the labels now name the *decision* (Send to Cast only) rather than the template (Send Successful only).
  - **Mark** (2026-08-09) — `mark_{offered|accepted}_{all|cast|alt|reject}`, the bulk twin of the single-invite modal's "Update Status Only". Records `offerStatus` (+ `placementResponse` for the accepted variants) and **sends nothing**. `parseInviteMode()` turns a value into `{kind, scope, recordAccepted}`, where `scope` is expressed in `selectInviteTargets`' existing vocabulary so a mark and a send over the same group always hit exactly the same applicants. The write is `applyStatusOnlyBulkLocked()` — one `withStorageLock` cycle, `applyStatusOnlyUpdate` per record, load inside the lock so each `castingStatus` is re-read immediately before its own stamp. A record that can't be updated is **skipped and reported**, never aborts the batch. On `mark_accepted_*`, Don't Cast applicants have no accepted state so they fall back to notified rather than erroring.
  - Both blocks pass through the same confirm card (`buildInvitesConfirm`), which flips its wording/colour/button label on `kind` — a mark says "no messages are sent", a send warns it pings people.
  - **REMOVED 2026-08-09:** `selected` ("Send to currently selected applicant only"). Bulk is only reachable from Marooning, where the button hardcodes `appIndex = 0`, so it always fired at whoever sorted first. The single-applicant modal keeps its own `selected` option.
- **✒️ Send Invite** (Casting card utility row, `casting_send_{appIndex}_{configId}`) → the **single variant** (`opts.single`): identical template fields, but the select shows **💾 Save as draft** · **📨 "Send {name} {Casting Offer\|Unsuccessful\|Alternate Message}"** (value `selected`) · **🕵️ Update Status Only - Offered** (Cast/Alternate) or **- Notified** (Don't Cast — no spot exists to "offer") (value `status_only`) · and, **Cast/Alternate only** (no accepted state exists for Don't Cast), **🎉 Update Status Only - Accepted** (value `status_only_accepted`) — records an accept the host confirmed *outside* CastBot (e.g. DM), writing `placementResponse` exactly as a real Accept click would. Both `status_only*` variants are the quiet/private bookkeeping path — neither renames the channel nor posts publicly, unlike a real Accept/Decline. The card button is **context-aware**: grey + disabled when no decision, else blue with the label **Send Offer** (cast) / **Send Decline** (reject) / **Send Alternate** (alternative).

Flow:
- **Submit** (`casting_messages_save:{appIndex}:{configId}`) always saves the templates (`@everyone`/`@here` neutralized). `draft` → "💾 saved". `status_only`/`status_only_accepted` → `applyStatusOnlyUpdate()` stamps `offerStatus` from the decision (`OFFER_FOR_STATUS`), and for the `_accepted` variant also `placementResponse` (`ACCEPTED_RESPONSE_FOR_STATUS`) — **without sending** — + returns a summary. Any send mode → an ephemeral **confirmation card** (counts per type) with Confirm/Cancel.
...
- `validatePlannerFields()` validates each estimate **independently** and saves partial progress — the *only* thing that invalidates a submit is a missing season name. `hasPlannerData` (all four supplied) gates **round generation**, not saving.
- `createSeason()` generates rounds when all four estimates arrive at once. `updateSeason()` **merges** estimates (a blank field never wipes a stored one) and regenerates rounds on first-time completion **or** when a *structural* estimate (players / swaps / FTC) changes — a name- or date-only edit leaves the timeline alone.

> Full detail — the estimate lifecycle, the day/date arithmetic, the round string selects, round-edit modals, and the Schedule/Calendar images — lives in **[SeasonPlanner.md](SeasonPlanner.md)**.
