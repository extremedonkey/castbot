...
A first-class Season Manager tab, rendered by `buildMarooningView`. Groups render in this order: **✅ Cast Players / 🔄 Alternate / ⚪ Undecided** (the "still in consideration" candidates — share ONE continuous numbering counter) → **🙅 Don't Cast / ✖️ Withdrawn** (each own numbering, restarting at 1; **hidden by default**, revealed via the **🗑️ Rejects** toggle button in the bottom row — `marooning_show_rejects_*` / `marooning_hide_rejects_*`, static label regardless of state). Cast Players and Alternate further break into **- Accepted / - Offer Sent / - Draft** sub-sections (`splitByOfferStage`); a Declined response stays folded into Offer Sent (the only place its `· 🚫 Declined` tag isn't redundant — suppressed inside "- Accepted" via `suppressAcceptedTag`). Every row is **one line**: `2. ReeceBot - 27yo | @Ask | @CST / CDT` — name plus inline demographics (`resolvePlayerDemographics`, shared with the Casting card's Overview block), with the `· 🎉 Accepted` / `· ✅ Accepted (Alt)` / `· 🚫 Declined` placement tags appended. **Scores and vote counts are NOT shown here** (removed 2026-08-09) — by marooning the decision is already made, so `5.0/5.0 (1 vote)` was noise, and pushing demographics onto a second `-#` line doubled the height of every roster. Ordering still runs highest-score-first via `computeCastingOrder`, so the ranking is preserved; the numbers live on the 🏆 Casting tab. Cast Players' header shows `(N/Est)` when the Season Planner's Estimated Number of Players is set (`estimatedTotalPlayers`, uncapped — exceeding it is valid). Grouping/sort is the shared `computeCastingOrder` (order: cast → alternative → undecided → reject → withdrawn; normalizes any unknown status → Undecided).
