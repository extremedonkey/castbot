...
**Status**: Live on **both surfaces** (2026-08-16) — any **Manage Channels/Manage Roles** admin. The Season Manager #️⃣ tab is open to everyone (the `CHANNEL_ADMIN_USER_IDS` whitelist is retired); on it, the four channel-fabrication buttons (Confessionals / Subs / 1on1s / Alliances) **premium lock-swap** for unentitled guilds while the 🎭 Player Roles row and Swap/Merge stay free. The Premium surface still lock-swaps its whole container. The test account (`1086246253819613274`) keeps only the player-facing alliance *request* flow (`ALLIANCE_REQUEST_USER_IDS`)
...
| 🤝 **1 on 1s** | A private channel for every *pair* of players in a tribe. **🗃️ Archive** (2026-08-17) sets pairs view-only — SendMessages moves allow→deny on every non-host overwrite (`viewOnlyOverwrites`, channelPlan.js; a member-level allow beats an @everyone deny, hence per-overwrite). Registry-driven by the recorded `tribeRoleId`, so it works after a swap rewrote the castlist; empty tribe selection = every recorded pair |
...
3. **Season Manager #️⃣ tab**: open to **everyone** since 2026-08-16 (the `CHANNEL_ADMIN_USER_IDS` whitelist is fully retired — `buildSeasonNavRow` always renders the tab and the Edit-origin round trip no longer re-checks). Its premium gate is `buildChannelsSection(configId, { entitled })`: unentitled guilds get `premium_locked_*` on exactly Confessionals / Subs / 1on1s / Alliances (the upsell handler serves the click); the 🎭 Player Roles row, Swap/Merge and Msg Category stay free. `entitled` = `hasPremiumAccessSync(guildId)` with the owner bypass, mirroring `buildPremiumMenu`.
