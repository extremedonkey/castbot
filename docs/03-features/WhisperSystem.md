...
- **Read receipts + received content (2026-07-18)**: opening a whisper fires `SAFARI_WHISPER_READ` (env log, guild Safari Log, spectator whisper channel) AND writes two activity entries via `logWhisperRead`: the **sender** gets `**Reader** opened your whisper` (no content echo), the **reader** gets `**Sender** whispered you` + a 150-char content preview (`formatWhisperPreview`). Whisper content therefore persists in: sender's activity log, reader's activity log, guild Safari Log, spectator channel — "one-time read" means one-time *interactive delivery*, not no-record.
- **🔒 PRIVACY INVARIANT**: player activity logs are viewable by their **owner + admins only** (`player_view_logs` is hard-wired to the clicker; `admin_view_logs_*` is ManageRoles-gated). Whisper entries are keyed strictly by verified ids (reader identity enforced by `handleReadWhisper`'s recipient check; sender from the stored whisper record). **Any future feature exposing activity logs to other players must exclude whisper entries.** Note: player-log whisper content is written unconditionally — there is currently no host toggle for it (guild `logTypes.whispers` only gates the Safari Log channel).
