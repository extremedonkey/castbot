...
> **Updated 2026-07-15 (later)**:
> - **Enhanced log format** — `safariLogSettings.logFormat: 'classic'|'enhanced'` (absent = classic, zero change). Selected via the "Set Log Type" select on the Logs config screen (`safari_log_format_select`), applies immediately. Enhanced mirrors the Player Activity Log style (`<t:..:R>` relative timestamps, `🗺️ **Movement** (A2) — **name** ...`, `> • ` action bullets, stamina code tags) — formatter: `logFormatter.js` (pure, unit-tested; item names injected from already-loaded safariData). Classic switch in `postToSafariLog` untouched. Init entries render as 🚀 Init under Enhanced (classic still shows "moved from null" — known wart).
> - **CastBot env logs (#🪵logs)** — reeces_stuff "CastBot Logs" button opens a config modal (`castbot_logs_modal`): enable/disable, per-env channel, ignored users (per-env `excludedUserIds`), Classic/Enhanced format (`liveDiscordLogging.format`, rendered by `formatEnhancedAnalyticsLine`), optional guild cache refresh (force member+role refetch). Writes go through `withStorageLock`; channel changes invalidate analyticsLogger's channel cache without restart.
>
> **Updated 2026-07-15**:
> - **Dedicated Whisper Log** — `safariLogSettings.whisperLogChannelId`: a second, spectator-safe channel receiving ONLY `SAFARI_WHISPER` posts, independent of `enabled`/`logTypes` (dual delivery with the main channel when both eligible). Configured via Map Explorer → 🤫 Whispers button (bottom-right; `whisper_log_config` — id must not start with `safari_whisper_`, that prefix routes to the player whisper flow). Targets resolved by `getSafariLogTargets()` (`safariLogger.js`), delivered per-channel with isolated error handling in `postToSafariLog()`.
