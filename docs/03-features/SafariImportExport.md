...
- **v2.2 grid fidelity (2026-07-26):** `filterMapsForExport` now exports resolved `gridWidth`/`gridHeight` alongside raw `gridSize` — `gridSize` can be a stale legacy value on re-imaged maps (a 7x4 map that began life as 2x2 still carried `gridSize: 2`, so its export auto-created a 2x2 map and silently dropped 24 of 28 cells). Import-side, `resolveImportGridDims(map, manifest)` (exported) sizes map auto-creation: package manifest's `mapImage` grid first (the packaged image is cropped to exactly that grid), else the map's resolved dims expanded — never shrunk — to the bounding box of its exported coordinates, which rescues pre-fix exports
...
- Implementation History:
  - [SafariImportMapIDMismatch.md](../02-implementation-wip/SafariImportMapIDMismatch.md) - Ghost map bug analysis
  - [SafariCustomActionsImportExport.md](../02-implementation-wip/SafariCustomActionsImportExport.md) - Original design doc
  - [SafariCustomActionsImportExport_GapsAnalysis.md](../02-implementation-wip/SafariCustomActionsImportExport_GapsAnalysis.md) - Gaps analysis
