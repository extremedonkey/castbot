...
- **Legacy `map_grid_edit_modal_` editor** (deprecated ActionRow path) validated a `cdn.discordapp.com` prefix — re-hosted URLs ARE `cdn.discordapp.com` attachments, so the two writers stayed compatible. (Deleted 2026-07-21 — it had zero emitters; see backlog item 8.)
...
| ✅ | Enemy image (was #1) | fieldEditors.js `createEnemyFieldModal` info group (via shared `buildImageFieldLabel`) | `enemy.image` (root) | Display-URL | Converted 2026-07-20 — same `entity_modal_submit_*` resolver gate as the pilot |
| ✅ | Rich Card image — shared def (was #4) | richCardUI.js `buildRichCardModal({ imageUploadMode })` | per consumer | Display-URL | Converted 2026-07-20; consumers must thread the mode + run `resolveUploadedImageField` with `currentValue` (extractRichCardValues returns image:'' on 0 files). Wired consumers: **challenges**, **channels msg composer**, **Category Post** (2026-07-21 — hardcodes `uploadComponent`, no paste-URL variant at all; the old richcard demo it replaced was retired, see [CategoryPost.md](../03-features/CategoryPost.md)) |
| ✅ | Dice Roll result images (was #5) | diceRoll.js `buildProbabilityModal(..., imageUploadMode)` | `condition.config.passResult/failResult.image` | Display-URL | Converted 2026-07-20 — **covers BOTH submits**: `prob_modal_*` and the `d20_result_modal_*` twin (the d20 opener reuses buildProbabilityModal — a site the original sweep missed). Submits stay non-deferred (showConditionEditor sends via res); sync re-host accepted for admin flows |
| ✅ | Challenge image (was #6) | challengeManager.js `buildChallengeModal(..., imageUploadMode)` | `challenges[id].image` | Display-URL | Converted 2026-07-20; submit now `deferred: true`; keep-current via `currentValue` (updateChallenge treats '' as clear) |
| ✅ | Tips showcase image (was #9) | app.js `edit_tip_` opener (via `buildImageFieldLabel`) + `save_tip_` submit | `tips[i].urls[env]` | Display-URL | Converted 2026-07-20; submit parses via `collectModalFields`; re-hosted (tips display forever — ephemeral URLs would rot); cdn guard untouched (re-hosted URLs pass it; guard only fires on non-empty pasted text) |
| ✅ | Custom Action "Display Text" image (was #2) | customActionUI.js `buildDisplayTextModal(..., imageUploadMode)` (extracted pure builder) | `action.config.image` | Display-URL | Converted 2026-07-21 — `handleDisplayTextSave` now parses via `collectModalFields` + `resolveUploadedImageField` (fieldKey `action_image`, keep-current via `currentValue`); sync re-host (submit sends via res, dice precedent) |
| ✅ | Legacy display-text twin (was #3) | ~~app.js `safari_action_modal_*_display_text`~~ **RETIRED 2026-07-21** | same data as #2 | — | `safari_add_action_*_display_text` now delegates to the modern `buildDisplayTextModal` (next actionIndex computed); the legacy submit case bails with a "reopen the action" message for stale modals. Also fixed the shape drift (legacy wrote `imageUrl`/`accentColor`, modern writes `image`/`color`) |
| ✅ | Season App question image (was #7) | applicationManager.js `buildQuestionModal({..., imageUploadMode})` — replaced FIVE inline copies (question_add_, question_select_ edit, completion edit + two legacy stale-message openers) | `question.imageURL` | Display-URL | Converted 2026-07-21; both submits (`season_new_question_modal_`, `season_edit_question_modal_`) parse via `collectModalFields` + resolver (fieldKey `imageURL`, edit keeps current on 0 files) |
| ✅ | Legacy map cell editor (was #8) | ~~`map_grid_edit_` / `map_grid_edit_modal_`~~ **DELETED 2026-07-21** | `baseContent.image` | — | Verified zero emitters (nothing ever created a `map_grid_edit_{coord}` button) — dead code, both blocks removed (~200 lines). Legacy handler baseline 102→100, app.js ratchet 52850→52458 |

**Backlog complete** — every image-URL modal input found in the 2026-07-20 sweep is now converted or retired. Remaining paste-URL surfaces are non-modal (entity metadata defs) or deliberately text-only.

**Shared machinery added in the 2026-07-20 batch** (`src/images/modalImageUpload.js`): `buildImageFieldLabel()` — one Label builder for every converted field (text vs upload mode, `Current: name.png` state in the description); `resolveUploadedImageField()` generalized with `fieldKey` (e.g. `result_image`, `image_url`) and `currentValue` (explicit keep-current for handlers that treat `''` as clear).
