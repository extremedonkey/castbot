...
| ✅ | Rich Card image — shared def (was #4) | richCardUI.js `buildRichCardModal({ imageUploadMode })` | per consumer | Display-URL | Converted 2026-07-20; consumers must thread the mode + run `resolveUploadedImageField` with `currentValue` (extractRichCardValues returns image:'' on 0 files). Wired consumers: **challenges**, **channels msg composer**. The richcard demo (app.js `richcard_demo_edit/save`) deliberately stays text-only (throwaway card, sync submit) |
