...
**Module:** `src/monitoring/restartScheduler.js`, patterned on healthMonitor (config persistence in `playerData.environmentConfig`) and backupService (chained setTimeout). The warning posts via **raw REST** (`DiscordRequest`) — the prodWatchdog `channel.send` pattern the original design cited turned out NOT to be proven: discord.js v14.16's MessagePayload rejects raw CV2 containers (`ActionRowBuilder is not a constructor`, hit live 2026-07-06), and prodWatchdog silently falls back to a webhook ping on that error, masking it. Documented as [ComponentsV2Issues.md #15](../troubleshooting/ComponentsV2Issues.md).

> **Discovered follow-up (not fixed here):** ProdWatchdog's rich alert path (CV2 + "Restart Prod Now" button via `channel.send`) most likely **never fires** — every alert has been taking the plain webhook-ping fallback. Same one-line class of fix (DiscordRequest REST) if/when we want the button back on watchdog alerts.
