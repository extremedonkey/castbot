 ...
 *
 * Concepts:
 * 1. Vertical Timeline — round-by-round with event markers and date cards
 * 2. Month Calendar — traditional calendar grid with rounds highlighted
 * 3. Gantt-style — horizontal bars showing round duration and type
 * 4. Countdown Strip — compact horizontal strip with F-numbers and milestones
 */
...
function getRoundLabel(round) {
  ...
  if (type === 'ftc') return 'Final Tribal';
  if (type === 'marooning') return 'Marooning';
  if (type === 'swap') return round.eventLabel || 'Swap';
  if (type === 'merge') return round.eventLabel || 'Merge';
  return round.challengeName || `Challenge ${round.seasonRoundNo}`;
}
...
export async function generateVerticalTimeline(seasonName, rounds, startDate) {
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    const label = escapeXml(stripEmoji(getRoundLabel(round)));
...
export async function generateMonthCalendar(seasonName, rounds, startDate) {
  ...
  const HEIGHT = TITLE_H + months.length * monthHeight + MARGIN;

  const composites = [];
  const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
  const dayNames = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

  // Build lookup: date string → round info
  const dateLookup = {};
  for (const id of sortedIds) {
    const round = rounds[id];
    const dateInfo = dates[id];
    ...
    for (let day = 0; day < dur; day++) {
      ...
      dateLookup[key] = { round, id, dayInRound: day, type: getRoundType(round) };
    }
  }
  ...
  composites.push({
    input: Buffer.from(`<svg width="${WIDTH}" height="${TITLE_H}" xmlns="http://www.w3.org/2000/svg">
      <text x="${WIDTH / 2}" y="35" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="20" font-weight="bold" fill="${TEXT_PRI}">${escapeXml(stripEmoji(seasonName))} — Calendar</text>
      <text x="${WIDTH / 2}" y="55" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="12" fill="${TEXT_MUT}">${formatDateShort(startDate)} – ${formatDateShort(endDate)} | ${dates._totalDays} days</text>
    </svg>`),
    top: 0, left: 0
  });
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (info) {
        accentColor = TYPE_COLORS[info.type];
        cellBg = CARD_ALT;
        roundText = `F${info.round.fNumber}`;
      }
      ...
      composites.push({
        input: Buffer.from(`<svg width="${CELL_W}" height="${CELL_H}" xmlns="http://www.w3.org/2000/svg">
          ...
          ${roundText ? `<text x="${CELL_W / 2}" y="48" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="11" font-weight="bold" fill="${accentColor}">${roundText}</text>` : ''}
        </svg>`),
        top: cy, left: cx
      });
    }
...
// ═══════════════════════════════════════════
// CONCEPT 3: Gantt Chart
// ═══════════════════════════════════════════

export async function generateGanttChart(seasonName, rounds, startDate) {
  const sortedIds = Object.keys(rounds).sort((a, b) => rounds[a].seasonRoundNo - rounds[b].seasonRoundNo);
  const dates = calcDates(rounds, startDate);
  const totalDays = dates._totalDays;

  const MARGIN = 30;
  const LABEL_W = 100;
  const ROW_H = 28;
  const HEADER_H = 80;
  const CHART_W = Math.max(500, Math.min(900, totalDays * 18));
  const WIDTH = MARGIN + LABEL_W + CHART_W + MARGIN;
  const HEIGHT = HEADER_H + sortedIds.length * ROW_H + MARGIN + 30; // 30 for date axis
  ...
  composites.push({
    input: Buffer.from(`<svg width="${WIDTH}" height="${HEADER_H}" xmlns="http://www.w3.org/2000/svg">
      <text x="${WIDTH / 2}" y="32" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="20" font-weight="bold" fill="${TEXT_PRI}">${escapeXml(stripEmoji(seasonName))} — Gantt</text>
      <text x="${WIDTH / 2}" y="52" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="12" fill="${TEXT_MUT}">${sortedIds.length} rounds over ${totalDays} days</text>
    </svg>`),
    top: 0, left: 0
  });

  // Date axis (tick marks every 7 days)
  const axisSvgParts = [];
  for (let day = 0; day <= totalDays; day += 7) {
    const x = MARGIN + LABEL_W + day * dayW;
    const tickDate = new Date(startDate);
    tickDate.setDate(tickDate.getDate() + day);
    axisSvgParts.push(`<text x="${x}" y="15" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="9" fill="${TEXT_MUT}">${formatDateShort(tickDate)}</text>`);
    axisSvgParts.push(`<line x1="${x}" y1="18" x2="${x}" y2="${sortedIds.length * ROW_H + 18}" stroke="${SEPARATOR}" stroke-width="0.5" stroke-dasharray="3,3"/>`);
  }
  composites.push({
    input: Buffer.from(`<svg width="${WIDTH}" height="${sortedIds.length * ROW_H + 25}" xmlns="http://www.w3.org/2000/svg">${axisSvgParts.join('')}</svg>`),
    top: HEADER_H, left: 0
  });
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    const color = TYPE_COLORS[type];
    const dur = getRoundDuration(round);
    const offset = dates[id].offset;
    const y = HEADER_H + 20 + i * ROW_H;
    const barX = MARGIN + LABEL_W + offset * dayW;
    const barW = Math.max(dur * dayW - 2, 4);

    composites.push({
      input: Buffer.from(`<svg width="${WIDTH}" height="${ROW_H}" xmlns="http://www.w3.org/2000/svg">
        <text x="${MARGIN + LABEL_W - 10}" y="18" text-anchor="end" font-family="Arial, Helvetica, sans-serif" font-size="11" fill="${TEXT_SEC}">F${round.fNumber}</text>
        <rect x="${barX}" y="4" width="${barW}" height="18" rx="4" ry="4" fill="${color}" fill-opacity="0.8"/>
        ${barW > 40 ? `<text x="${barX + barW / 2}" y="17" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="9" font-weight="bold" fill="${BG}">${dur}d</text>` : ''}
      </svg>`),
      top: y, left: 0
    });
  }
  ...
  const legendTypes = ['marooning', 'swap', 'merge', 'standard', 'ftc', 'reunion'];
  const legendSvg = legendTypes.map((t, i) =>
    `<rect x="${MARGIN + i * 100}" y="4" width="12" height="12" rx="3" fill="${TYPE_COLORS[t]}"/>` +
    `<text x="${MARGIN + i * 100 + 18}" y="14" font-family="Arial, Helvetica, sans-serif" font-size="10" fill="${TEXT_MUT}">${t}</text>`
  ).join('');
  composites.push({
    input: Buffer.from(`<svg width="${WIDTH}" height="20" xmlns="http://www.w3.org/2000/svg">${legendSvg}</svg>`),
    top: legendY, left: 0
  });
...
export async function generateCountdownStrip(seasonName, rounds, startDate) {
  ...
  composites.push({
    input: Buffer.from(`<svg width="${WIDTH}" height="${HEADER_H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${WIDTH / 2}" y="48" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="12" fill="${TEXT_MUT}">F${rounds[sortedIds[0]].fNumber} → F${rounds[sortedIds[sortedIds.length - 1]].fNumber} | ${dates._totalDays} days</text>
    </svg>`),
    top: 0, left: 0
  });
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    // F-number below (alternating up/down for readability)
    ...
    // Event label above for special rounds
    if (isEvent) {
      const eventLabel = type === 'reunion' ? 'END' : type === 'ftc' ? 'FTC' : type === 'marooning' ? 'START' : type.toUpperCase();
      composites.push({
        input: Buffer.from(`<svg width="60" height="16" xmlns="http://www.w3.org/2000/svg">
          <text x="30" y="12" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="8" font-weight="bold" fill="${color}">${eventLabel}</text>
        </svg>`),
        top: lineY - NODE_R - 18,
        left: Math.round(cx - 30)
      });
